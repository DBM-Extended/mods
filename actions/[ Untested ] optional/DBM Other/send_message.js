module.exports = {
  ...
  meta: { version: "2.1.0", preciseCheck: false, author: null, authorUrl: null, downloadUrl: null },
  ...
  async action(cache) {
    ...
    if (Array.isArray(data.buttons)) {
      for (let i = 0; i < data.buttons.length; i++) {
        ...
        var btn_appear_or_not = this.evalMessage(button["appear_or_not"],cache) || "1"
        var btn_disabled_or_not = this.evalMessage(button["disabled_or_not"],cache) || "0"
        var btn_color_overwrite = this.evalMessage(button["color_overwrite"],cache) || undefined

        if (btn_appear_or_not !== "1" && btn_appear_or_not.toString().toLowerCase() !== "true" && btn_appear_or_not.toString().toLowerCase() !== "yes") continue;

        if (btn_disabled_or_not) {
          if (btn_disabled_or_not.toString() !== '1' && btn_disabled_or_not.toString().toLowerCase() !== 'yes' && btn_disabled_or_not.toString().toLowerCase() !== 'true') finalstep(0) 
          else finalstep(1)
        } else if (!btn_disabled_or_not) {
          if (button.id.includes("disabled") === true) finalstep(1) 
          else finalstep(0)
        }
